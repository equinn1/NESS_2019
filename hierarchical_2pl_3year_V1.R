...
for (i in seq(1:J)){
  theta_sigma = matrix(c(1, 0.3, 0.2, 0.3, 1, 0.2,0.2,0.2,1), ncol = 3)
...
LKJ_eta1 = 4   #parameters for LKJ prior for difficulty and discrimination
LKJ_eta2 = 25
...
theta1=vector('numeric')
theta2=vector('numeric')
theta3=vector('numeric')
for (j in 1:J){
  theta1 <- c(theta1,mean(pd$mu_theta[,j,1]))
  theta2 <- c(theta2,mean(pd$mu_theta[,j,2]))
  theta3 <- c(theta3,mean(pd$mu_theta[,j,3]))
}
theta1a <- theta_m[,1]
theta2a <- theta_m[,2]
theta3a <- theta_m[,3]
